...
if [[ "${BUILD_IMAGES}" == "true" ]]; then
    ...
    run "(10/11) Building builder-core" \
        "docker run -t --rm --privileged -v $(pwd):/work alpine sh -c 'sed -i 's/dl-cdn.alpinelinux.org/mirrors.ustc.edu.cn/g' /etc/apk/repositories && apk update && apk add --no-cache rsync && \
        cd /work && \
